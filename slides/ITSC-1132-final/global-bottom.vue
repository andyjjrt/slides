...
const distribution = computed(
  () => (formatter.value.glow || $slidev.themeConfigs.glow || "full") as Distribution
);
const opacity = computed<number>(() => +( formatter.value.glowOpacity ?? $slidev.themeConfigs.glowOpacity ?? 0.4));
const hue = computed<number>(() => +( formatter.value.glowHue || $slidev.themeConfigs.glowHue || 0));
const seed = computed<string>(() =>
  formatter.value.glowSeed === "false" || formatter.value.glowSeed === false || $slidev.themeConfigs.glowSeed === "false" || $slidev.themeConfigs.glowSeed as any === false
    ? Date.now().toString()
    : formatter.value.glowSeed || $slidev.themeConfigs.glowSeed || "default"
);


const exportMode = computed(() => {
  return (import.meta as any).env.VITE_EXPORT
})
